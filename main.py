...
class StepikClient:
    ...
    def _save_tokens(self, tokens: Dict[str, Any]):
        ...
        # Выводим информацию о scopes для отладки
        if 'scope' in tokens:
    ...
    def _authorize_user_manual(self) -> Optional[str]:
        # КРИТИЧЕСКИ ВАЖНО: используем правильный scope
        params = {
            'response_type': 'code',
            'client_id': self.client_id,
            'redirect_uri': self.REDIRECT_URI,
            'scope': 'read write'  # Это даёт права на чтение И запись (включая enrollments)
        }
    ...
    def enroll_in_course(self, course_id: int) -> bool:
        ...
        # Правильный формат данных для Stepik API
        payload = {
            "enrollment": {
    ...
    def process_course(self, course: Dict[str, Any]):
        ...
        # ВАЖНО: Проверяем и записываемся на курс
        ...
        if not is_enrolled:
            ...
            if not enrolled:
                ...
                # Пауза для обработки на сервере
                time.sleep(1)
        else:
...
if __name__ == '__main__':
    try:
        client = StepikClient()
        print("\n" + "="*80)
        print("ТЕСТИРОВАНИЕ: Попытка доступа к курсу 214747")
        print("="*80)
        
        courses = client.fetch_objects('courses', [214747])
        if courses:
            for c in courses:
